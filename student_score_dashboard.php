...
if ($result_info && mysqli_num_rows($result_info) > 0) {
    // Fetch all titles associated with the student
    $titles = array();
    while ($row_info = mysqli_fetch_assoc($result_info)) {
        $titles[] = $row_info['title_name'];
    }
} else {
        ...
        <div class="side-menu" id="sideMenu">
            <ul>
                <?php foreach ($titles as $title) : ?>
                    <li><a href="student_score.php?teacherid=<?php echo $teacherid; ?>"><i class="fas fa-pencil-alt"></i>&nbsp;<span><?php echo $title; ?></span></a></li>
                <?php endforeach; ?>
            </ul>
        </div>
